...
async function fetchProjectDetails(req, res) {
  try {
    ...
    res
      .status(500)
      .json({ error: { message: "Failed to fetch project details." } });
    return null;
  }
}
...
function sendProjectDetails(res, project) {
  try {
    ...
    res
      .status(500)
      .json({ error: { message: "Failed to send project details." } });
  }
}
...
handler.post(
  validateBody({
    ...
    required: ["name", "apiUrl", "apiVersion", "apiPrefix"],

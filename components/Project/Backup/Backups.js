...
import { Card, Alert, Button, Tooltip, Space, Typography } from 'antd';
...
const reducer = (state, action) => {
    ...
    const updateBackup = () => {
        let index = state.backups.findIndex(backup => backup._id === action.backup?._id);
        if (index !== -1) {
    ...
    switch (action.type) {
        ...
        case "add":
            return {
                ...state, backups: [...state.backups, action.backup],
        ...
        case "update":
            ...
            updateBackup();
            return { ...state, backups: [...state.backups], total: state.total + 1, error: '' }
        case "delete":
            deleteBackup();
            return { ...state, backups: [...state.backups], total: state.total - 1, error: '' }
        case "error":
...
export default function Backups({ projectId }) {
    ...
    const onSubmit = async (states) => {
        ...
        try {
            ...
            const res = await BackupServices.createOrUpdate(projectId, backup?._id, states)

            dispatch({ type: backup?._id !== -1 ? 'update' : 'add', backup: res.backup });
    ...
    return <>
        <Card loading={state.loading} title={`Backups:`}
            extra={state.total > 0 && <Tooltip title="Add New Backup"><Button
                type="text"
                onClick={(e) => {
                    e.preventDefault();

...
const reducer = (state, action) => {
    ...
    switch (action.type) {
        ...
        case "add":
            return { ...state, roles: [...state.roles, action.role], total: state.total + 1, error: '' }
        case "edit":
        ...
        case "update":
            ...
            updateRole();
            return { ...state, roles: [...state.roles], total: state.total + 1, error: '' }
        case "delete":
            deleteRole();
...
export default function Users({ projectId, setUser }) {
    ...
    const onSubmit = async (states) => {
        ...
        try {
            ...
            const res = await UserServices.createOrUpdate(projectId, role?.id, states)
    ...
    return <>
        <Card loading={state.loading} title={`Users:`}
            extra={state.total > 0 && <Tooltip title="Add New User"><Button
                type="text"
                onClick={(e) => {
                    e.preventDefault();
                    onEdit({ ...initialRole });
        ...
        >
            ...
            {state.total <= 0 ? <Space size={20} align="center">

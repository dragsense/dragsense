...
export default function Documents({ collection, form }) {
  ...
  const load = async () => {
    ...
    try {
      const collectionRes = form
        ? await FormServices.get(collection._id)
        : await CollectionServices.get(collection._id);
      ...
      if (res.documents) {
        const data = Array.isArray(res.documents.results)
  ...
  const search = async () => {
    try {
      ...
      const res = await DocumnetServices.search(
        collection._id,
        searchQuery,
        form
      );
  ...
  const onSubmit = async (states) => {
    ...
    try {
      ...
      const res = await DocumnetServices.createOrUpdate(
        collection._id,
        document?._id,
        states,
        form
      ...
      dispatch({
        type: document?._id !== -1 ? "update" : "add",
      ...
      <Card
        ...
        extra={
          state.document ? (
            ...
            state.total > 0 && (
              <Tooltip title="Add New Documnet">
                <Button
                  type="text"
                  onClick={() =>
                    onEdit({

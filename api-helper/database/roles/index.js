...
export async function findRolesByProjectRoles(
  db,
  search,
  roleIds,
  page = 0,
...
export async function deleteRolesByProjectRoles(db, roleIds) {
  await db.collection("roles").aggregate([
    {
  ...
  return await db.collection("roles").deleteMany({ _id: { $in: roleIds } });
}

export async function findRolesByUserProjects(
  db,
  roleIds,
  search,
  page = 1,
...
export async function insertRole(
  db,
  { name, userId, projectId, permissions = [] }
) {
  const role = {
    permissions,
    name,

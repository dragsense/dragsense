...
const ProjectServices = {
    ...
    getCookie: async function (projectId) {
        try {
            return await fetcher(`/api/projects/${projectId}/cookie`, createFetchOptions('GET'));
        } catch (error) {
            handleFetchError(error);
        }
    },
    ...
    getAll: async function (page, limit, shared = false) {
        try {
            return await fetcher(`/api/projects${shared ? '/shared' : ''}?limit=${limit}&page=${page}`, createFetchOptions('GET'));
        } catch (error) {
            handleFetchError(error);
        }
    },
    ...
    search: async function (search, shared = false) {
        try {
            return await fetcher(`/api/projects${shared ? '/shared' : ''}/search?search=${search}`, createFetchOptions('GET'));
        } catch (error) {
            handleFetchError(error);
        }
    },
    ...
    createOrUpdate: async function (id, states) {
        try {
            return fetcher(`/api/projects${id !== -1 ? '/' + id : ''}`, createFetchOptions('POST', { 'Content-Type': 'application/json' }, { ...states }));
        } catch (error) {
            handleFetchError(error);
        }
    },
    ...
    download: async function (id, states) {
        try {
            const res = await fetcher(`/api/projects/${id}`, createFetchOptions('GET'));
    ...
    search: async function (search, page, limit) {
        try {
            ...
            const res = await fetcher(`/api/projects/${projectId}`, createFetchOptions('GET'));
            ...
            return await fetcher(`${project.url}/api/search?limit=${limit}&page=${page}`, createFetchOptions('POST', { 'Content-Type': 'application/json', 'x-api-key': project.apikey }, { search }));
        } catch (error) {
            handleFetchError(error);
        }
    },
    ...
    conenction: async function (project) {
        try {
            return await fetcher(`${project.url}`, createFetchOptions('GET', { 'x-api-key': project.apikey }));
        } catch (error) {
            handleFetchError(error);
        }
    },

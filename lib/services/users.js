...
const UserServices = {
    ...
    getAllUsers: async function (page, limit = 10) {
        try {
            return await fetcher(`/api/users?limit=${limit}&page=${page}`, {
                method: 'GET'
            });
        } catch (error) {
            handleFetchError(error);
    ...
    searchUsers: async function (search, page, limit = 10) {
        try {
            return await fetcher(`/api/users/search?search=${search}&limit=${limit}&page=${page}`, {
                method: 'GET'
            });
        } catch (error) {
            handleFetchError(error);
    ...
    search: async function (projectId, search, page, limit = 10) {
        try {
            return await fetcher(constructUrl(projectId, `users/search?search=${search}&limit=${limit}&page=${page}`), {
                method: 'GET'
            });
        } catch (error) {
            handleFetchError(error);
